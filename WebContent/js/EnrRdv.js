$(document).ready(function () {
            ...
            $('.btnSubmit').click(
                    function (event) {
                        ...
                        // def des regex
                        let regexDate = /^([0-2][0-9]|(3)[0-1])(\/)(((0)[0-9])|((1)[0-2]))(\/)\d{4}$/;
                        let regexHeure = /([0-9]{2}:[0-9]{2})/;

                        // laisser bootstrap gerer tant qu'il n'y pas toutes les
                        // infos
                        if (!$.isEmptyObject($('#date').val()) && !$.isEmptyObject($('#heure').val())
                                && !$.isEmptyObject($('#idMedecin').val())) {
                            if (!regexDate.test($('#heure').val()) || !regexDate.test($('#date').val())) {
